...
const NFTCard = ({ nftItem, title, listings }) => {
  ...
  useEffect(() => {
    const listing = listings[nftItem.id]
    // console.log(listing)
    // listings.find((listing) => listing.asset.id === nftItem.id)
    if (Boolean(listing)) {
      setIsListed(true)
  ...
  // console.log(nftItem.image)
  // await nftItem to be loaded
  if (!nftItem.image) return null
